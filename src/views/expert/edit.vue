...
import expert from '../../graphql/expert.gql';
import addExpertAdmin from '../../graphql/addExpertAdmin.gql';
import updateExpertAdmin from '../../graphql/updateExpertAdmin.gql';
...
export default {
  ...
  methods: {
    ...
    async created(id, callback) {
      ...
      callback(response.data && response.data.expert);
    },
    async update(successCallback, failureCallback) {
      ...
      result && result.data && result.data.updateExpertAdmin ? successCallback() : failureCallback();
    },
    async add(successCallback, failureCallback) {
      ...
      const result = await this.$apollo.mutate(
        {
          mutation: addExpertAdmin,
          variables: {
            expertInput: data,
          },
        },
      );
      if (result && result.data && result.data.addExpertAdmin) {
        this.item = result && result.data && result.data.addExpertAdmin;
        this.mode = 'EDIT';
        successCallback();
      } else {
        failureCallback();
    ...
    checkName(event) {
      if (this.experts && this.item.name) {
        const fuse = new Fuse(this.experts, {
          keys: ['name'],
        });
